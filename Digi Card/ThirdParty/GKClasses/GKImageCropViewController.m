...
- (void)_setupNavigationBar{
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Choose", @"")
...
- (void)_setupUseButton{
    ...
    if (floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_6_1) {
        ...
        [self.useButton setTitle:NSLocalizedString(@"Choose",@"") forState:UIControlStateNormal];
...
- (void)_setupToolbar{
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) {
        ...
        if (floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_6_1) {
            ...
            info.text = NSLocalizedString(@"MoveAndScale", @"");
        }
...
- (void)viewDidLoad{
    ...
    self.title = NSLocalizedString(@"ChoosePhoto", @"");
