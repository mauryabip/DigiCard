...
#define OKBUTTON_BACKGROUND_COLOR [UIColor colorWithRed:05.0 / 255.0 green:173.0 / 255.0 blue:152.0 / 255.0 alpha:1.0]
...
NSInteger const HHAlertview_SIZE_HEIGHT = 210;//250
...
- (void)configtext:(NSString *)title detail:(NSString *)detail
{
    ...
    [_titleLabel setFont:[UIFont fontWithName:@"HelveticaNeue-Medium" size:23.0f]];//[UIFont systemFontOfSize:HHAlertview_SIZE_TITLE_FONT]];
    [_titleLabel setTextAlignment:NSTextAlignmentCenter];
    [self addSubview:_titleLabel];
    
    if (_detailLabel==nil) {
        _detailLabel  = [[UILabel alloc] initWithFrame:CGRectMake(5, Simble_SIZE+Simble_TOP+HHAlertview_SIZE_TITLE_FONT+25, [self getSelfSize].width-10, HHAlertview_SIZE_TITLE_FONT)];
    }
    
  
    _detailLabel.textColor = [UIColor grayColor];
    [_detailLabel setFont:[UIFont fontWithName:@"HelveticaNeue" size:16.0f]];//[UIFont systemFontOfSize:HHAlertview_SIZE_DETAIL_FONT]];
    ...
    CGRect ReframeDetailLbl = _detailLabel.frame;
    ReframeDetailLbl.size.width =[self getSelfSize].width-10;
    _detailLabel.frame = ReframeDetailLbl;
    [self addSubview:_detailLabel];
    
    CGRect mainFrame= self.frame;
    mainFrame.size.height =_detailLabel.frame.size.height + _detailLabel.frame.origin.y+20;
...
- (void)configButton:(NSString *)cancel Okbutton:(NSString *)ok
{
    if (cancel==nil && ok==nil) {
        ...
        [self HideHHALertview];
    
    } else if (cancel==nil) {
        if (_OkButton==nil) {
            _OkButton = [[UIButton alloc] initWithFrame:CGRectMake(([self getSelfSize].width-Button_SIZE_WIDTH)/2, [self getSelfSize].height-Buutton_SIZE_HEIGHT-20, Button_SIZE_WIDTH, Buutton_SIZE_HEIGHT)];
        }
        else
        {
            [_OkButton setFrame:CGRectMake(([self getSelfSize].width-Button_SIZE_WIDTH)/2, [self getSelfSize].height-Buutton_SIZE_HEIGHT-20, Button_SIZE_WIDTH, Buutton_SIZE_HEIGHT)];
        }
        
        [_OkButton setTitle:ok forState:UIControlStateNormal];
        [_OkButton setBackgroundColor:OKBUTTON_BACKGROUND_COLOR];
        [_OkButton setBackgroundColor:[UIColor colorWithRed:05.0 / 255.0 green:173.0 / 255.0 blue:152.0 / 255.0 alpha:1.0]];
...
- (void)drawTick
{
    ...
    CAShapeLayer *layer = [[CAShapeLayer alloc] init];
    layer.fillColor = [UIColor clearColor].CGColor;
   // layer.strokeColor = [UIColor greenColor].CGColor;//Ashutosh Change
    layer.strokeColor = [UIColor colorWithRed:05.0 / 255.0 green:173.0 / 255.0 blue:152.0 / 255.0 alpha:1.0].CGColor;//Ashutosh Change
    layer.lineWidth = 5;
    layer.path = path.CGPath;
...
#pragma mark - hide dialog
-(void)HideHHALertview{
//    NSLog(@"Enter in HideHHALertview");
    [NSTimer scheduledTimerWithTimeInterval:2.0 target:self selector:@selector(HideView) userInfo:nil repeats:NO];
    
}
-(void)HideView{
//   NSLog(@"Enter in HideView");
  [HHAlertView Hide];
}
